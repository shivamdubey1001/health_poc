...
def effective_as_of(index_date: date | None = None) -> date:
    """The horizon beyond which nothing may be read.

    A backtest sets an index date in the past and every window is computed from
    it, so the model sees only what was knowable on that date. This is what makes
    real precision and recall available today rather than in ninety days."""
    return index_date or AS_OF
...
def get_recent_claims(db: Session, member_id: str, months: int | None = None,
                      index_date: date | None = None) -> list[dict]:
    runtime = get_runtime_settings(db)
    months = months or runtime["claim_lookback_months"]
    horizon = effective_as_of(index_date)
    cutoff = (horizon - timedelta(days=months * 30)).isoformat()
    rows = (
        db.query(models.ClaimHistory)
        .filter(
            models.ClaimHistory.member_id == member_id,
            models.ClaimHistory.service_from_date >= cutoff,
            models.ClaimHistory.service_from_date <= horizon.isoformat(),
        )
...
def get_recent_calls(db: Session, member_id: str, limit: int | None = None,
                     index_date: date | None = None) -> list[dict]:
    runtime = get_runtime_settings(db)
    ...
    calls = (
        ...
        .filter(
            models.MemberAdvocateCall.member_id == member_id,
            models.MemberAdvocateCall.call_start_timestamp <= f"{horizon.isoformat()}T23:59:59",
        )
        .order_by(models.MemberAdvocateCall.call_start_timestamp.desc())
        .limit(limit)
        .all()
...
def build_member_context(db: Session, member_id: str, index_date: date | None = None) -> dict | None:
    profile = get_member_profile(db, member_id)
    if not profile:
        return None
    plan = db.get(models.PlanMaster, profile["plan_id"])
    accumulator = db.get(models.BenefitAccumulator, member_id)
    claims = get_recent_claims(db, member_id, index_date=index_date)
    calls = get_recent_calls(db, member_id, index_date=index_date)
    return {
        "member": profile,
        "plan": _row(plan, [
