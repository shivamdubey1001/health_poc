...
export type Readiness = { member_id:string; predicted_care_event:string; readiness_score:number; readiness_label:string; checklist:ReadinessItem[]; top_issue:string|null; score_explanation:string[]; alternatives:Alternative[]; recommended_next_action:string; generated_by:string }
...
export type HealthStatus = { status:string; service:string; openai_configured:boolean; model:string; data_as_of:string; mock_ai:boolean }
export type CareBatchRow = { member: Member; assessment: CareIntent }
export type CareBatchResult = { selected_count:number; model:string; threshold:number; results:CareBatchRow[] }
