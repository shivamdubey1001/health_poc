...
def estimate_tokens(text: str) -> int:
    # Fallback estimator only. Real OpenAI mode uses API-reported token usage when provided.
...
def aggregate_usage(db: Session) -> dict:
    ...
    return {
        ...
        "pricing_note": f"Estimated from API-reported token usage and configured rates for {settings.openai_model}; update environment pricing if the model changes.",
    }
