...
echo.
REM Re-use an existing key if backend\.env already has one, so re-running the
REM launcher does not force the key to be pasted again.
set "REUSE_KEY="
if exist "%BACKEND_DIR%\.env" (
    findstr /R /C:"^OPENAI_API_KEY=sk-" "%BACKEND_DIR%\.env" >nul 2>&1
    if not errorlevel 1 (
        echo An OpenAI API key is already saved in backend\.env.
        choice /C YN /N /M "Keep the existing key? [Y/N] "
        if not errorlevel 2 set "REUSE_KEY=1"
    )
)
if defined REUSE_KEY (
    echo Keeping the existing key in backend\.env.
    call :sync_env_defaults
) else (
...
:sync_env_defaults
REM Add any settings introduced since this .env was written, without touching
REM the saved key.
powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "    $name=$Matches[1];" ^
  "    if($name -ne 'OPENAI_API_KEY' -and $text -notmatch ('(?m)^'+$name+'=')){$text=$text.TrimEnd()+\"`n$line`n\"}" ^
  "  }" ^
  "}" ^
  "[IO.File]::WriteAllText($dst,$text,(New-Object Text.UTF8Encoding($false)));" ^
  "Write-Host 'Environment settings are up to date.' -ForegroundColor Green"
exit /b 0
...
powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "$text=[regex]::Replace($text,'(?m)^OPENAI_TEMPERATURE=.*$','OPENAI_TEMPERATURE=0.0');" ^
  "[IO.File]::WriteAllText($dst,$text,(New-Object Text.UTF8Encoding($false)));" ^
