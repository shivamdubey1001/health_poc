...
export default function AssessmentResultsPage(){
  ...
  return <>
    ...
    <div className="space-y-3">{(careBatch.results||[]).filter(r=>r&&r.member&&r.assessment).map(({member,assessment})=>{
      ...
      return <article key={member.member_id} className="card p-0 overflow-hidden">
        <div className="grid items-center gap-3 p-4 lg:grid-cols-[40px_1.2fr_1.5fr_.7fr_.8fr_.8fr_48px]">
          ...
          <div><p className="text-xs font-semibold uppercase tracking-wide text-muted">Contact likelihood</p><div className="mt-1 flex items-center gap-2"><StatusBadge status={assessment.advocate_contact.risk_level}/><span className="text-lg font-bold text-brand-dark">{Math.round((assessment.advocate_contact.confidence||0)*100)}%</span></div><p className="mt-0.5 text-[11px] text-muted">chance of contacting an advocate soon</p></div>
